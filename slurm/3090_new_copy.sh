...
# srun -n 1 -c 4 --gres=gpu:4 --chdir /HOME/scz0cqe/run/LLMCL --job-name=infER --kill-on-bad-exit=1 \
...
srun --gres=gpu:8 --chdir /HOME/scz0cqe/run/LLMCL --job-name=infER --kill-on-bad-exit=1 \
	/HOME/scz0cqe/run/.conda/envs/LLMCL/bin/python \
		/HOME/scz0cqe/run/LLMCL/acc_inference.py \
			...
			--adapter_checkpoint_dir /HOME/scz0cqe/run/LLMCL/outputs/models/ER_new/ER_lora_checkpoint_jecqa >/HOME/scz0cqe/run/LLMCL/outputs/models/ER_new/ER_lora_checkpoint_jecqa/infer_log_${SLURM_JOB_ID}.txt 2>&1
